...
library(Matrix)
library(glmnet)
quarterly <- c("GDPC1","PCECC96","GPDIC1","GCEC1","IPDBS","RCPHBS",
               "OPHPBS","TABSHNO","TLBSNNCB")
tc6 <- c("CPIAUCSL","CPIULFSL","CPIAPPSL","PCEPI","CPITRNSL","DSERRG3M086SBEA","MZMSL","TOTRESNS","BUSLOANS","NONREVSL",
         "PPICMM","DTCTHFNM","CES3000000008","CES0600000008","CUSR0000SA0L2",
         "DTCOLNVHFNM","DNDGRG3M086SBEA","CPIMEDSL","DDURRG3M086SBEA","M2SL","INVEST","CES2000000008",
         "CUSR0000SAS","CUSR0000SA0L5","CUSR0000SAC","M1SL")

tc5 <- c("INDPRO","IPDMAT","RPI","IPBUSEQ","W875RX1","DPCERA3M086SBEA","USTPU",	"CE16OV",	"CLF16OV",
         "IPNMAT","IPFINAL","IPFPNSS","IPFUELS","IPNCONGD","USFIRE",
         "USCONS","USGOOD","M2REAL","DMANEMP","IPMAT","IPB51222S","MANEMP","NDMANEMP","PAYEMS",
         "SRVPRD","USGOVT","IPMANSICS","USTRADE","IPDCONGD","IPCONGD",
         "CES1021000001","USWTRADE")

tc2 <- c("FEDFUNDS","TB3MS","gs1","gs5","gs10","TB6MS","UNRATE","AAA","BAA","UEMPMEAN","AWOTMAN")

tc1 <- c("T10YFFM","TB3SMFFM","TB6SMFFM","T1YFFM","BAAFFM","AAAFFM","T5YFFM")


variables <- c(quarterly,tc6,tc5,tc2,tc1)
...
for(i in set7){
  temp <- as.numeric((out[[i]][,3]))
  new[[i]] <- as.data.frame(temp[-c(1:9)])
}
...
variables1 <- c(tc6,tc5,tc2,tc1,"sp",quarterly) 
